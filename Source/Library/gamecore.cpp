...
namespace game_framework {
	...
	void CGameState::ShowInitProgress(int percent, string message)
	{
		...
		CBrush *pb, b(RGB(155, 155, 155));				// �e��� progress��
		...
		CBrush b2(RGB(255, 255, 255));					// �e���� progrss�i��
		pDC->SelectObject(&b2);
	...
	void CInteger::ShowBitmap()
	{
		...
		if (n >= 0) {
			MSB = n;
			nx = x + digit[0 + Type].GetWidth()*(NUMDIGITS - 1);
		}
		else {
			MSB = -n;
			nx = x + digit[0 + Type].GetWidth()*NUMDIGITS;
		}
