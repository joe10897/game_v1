...
namespace game_framework
{
	...
	void ScoreBoard::ShowStars()
	{
		...
		int star_X = scoreBoard.GetLeft() + 159;

		thirdStar->SetTopLeft(star_X, scoreBoard.GetTop() + (281 - yellowStar.GetHeight() / 2));
		thirdStar->ShowBitmap();
		secondStar->SetTopLeft(star_X, (int)(scoreBoard.GetTop() + 281 + (((threeStar - twoStar) / threeStar) * 254 - greenStar.GetHeight() / 2)));
		secondStar->ShowBitmap();
		firstStar->SetTopLeft(star_X, (int)(scoreBoard.GetTop() + 281 + (((threeStar - oneStar) / threeStar) * 254 - redStar.GetHeight() / 2)));
	...
	void ScoreBoard::ShowMoves()
	{
		...
		{
			moves.SetDigit(size);
			moves.SetTopLeft(scoreBoard.GetLeft() + 119 - 9 * size, scoreBoard.GetTop() + 240);
			moves.ShowBitmap();
		}
	}
	...
	{
		int X_point = scoreBoard.GetLeft() + 159, Y_point = scoreBoard.GetTop() + 539;
	...
	void ScoreBoard::ShowScore()
	{
		...
		{
			score.SetDigit(size);
			score.SetTopLeft((scoreBoard.GetLeft() + 144 - (18 * size)), scoreBoard.GetTop() + 325);
			score.ShowBitmap();
		}
	}
